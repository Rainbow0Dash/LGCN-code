...
class BSDS_Loader(data.Dataset):
    ...
    def __init__(self, root=r'E:\pidinet-master\data\BSDS500\HED-BSDS', split='train', transform=False, threshold=0.3, ablation=False):
        self.root = root
        self.split = split
...
class BSDS_VOCLoader(data.Dataset):
    ...
    def __init__(self, root='pidinet-master/data/BSDS500', split='train', transform=False, threshold=0.3,
                 ablation=False):
        self.root = root
        self.split = split
        self.threshold = threshold * 256
    ...
    def __getitem__(self, index):
        if self.split == "train":
            ...
            lb = lb[np.newaxis, :, :]
            lb[lb == 0] = 0
            lb[np.logical_and(lb > 0, lb < threshold)] = 0.001
            lb[lb >= threshold] = 1
...
class UDED_Loader(data.Dataset):
    """
    Dataloader for Multicue
    """

    def __init__(self, root=r'E:\Eevee\newnet\UDED-main', split='test', transform=False, threshold=0.3, setting=['boundary', '1']):
        """
...
class NYUD_Loader(data.Dataset):
    ...
    def __init__(self, root=r'E:\pidinet-master\wget\NYUD', split='train', transform=False, threshold=0.4, setting=['image']):
        """
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            img_name = Path(img_file).stem
            lb = np.array(Image.open(os.path.join(r'E:\pidinet-master\wget\NYUD\test\contour\test', img_name + '.jpg')),
                          dtype=np.float32)
            if lb.ndim == 3:
                lb = np.squeeze(lb[:, :, 0])
            assert lb.ndim == 2
